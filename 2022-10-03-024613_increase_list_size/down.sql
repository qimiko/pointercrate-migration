        ...
        SELECT pseudo_records.player,
			...
			FROM (
                 SELECT player,
                        progress,
                        position,
                        CASE WHEN demons.position > 50 THEN 100 ELSE requirement END AS requirement
                 FROM records
                          INNER JOIN demons
                                     ON demons.id = demon
                 WHERE demons.position <= 100 AND status_ = 'APPROVED' AND (demons.position <= 50 OR progress = 100)
